...
//				cmd_readptr=cmd_readbuf;
//				memset(cmd_readbuf, 0, sizeof(cmd_readbuf));
//				LogEntry("Serial buffer overrun");
//			}
//			else
//			{
...
int GPSStartupCmds(void)
{
	...
	if(setting != NULL)
	{
		...
		if(str != NULL)
		{
			...
			LogEntry("GPS config file:");
			LogEntry(configfile);
		}
		else
		{
			fprintf(stderr, "%s: Invalid GPS config file %s\r\n",MODULE_NAME, str);
			LogEntry("Invalid GPS config file:");
			return(BCERROR);
		}
		if(configfile)
		{
			...
			if(!cmdfile)
			{
				...
				LogEntry("Error opening GPS config file:");
				LogEntry(configfile);
				return(BCERROR);
			}
...
// getline allocates memory when cmdstr = NULL, to allow unknown length strings
	i = 0;
	LogEntry("Sending GPS Startup Commands");
	while((numread = getline(&cmdstr, &len, cmdfile)) != -1)
	{
		LogEntry(cmdstr);
		if(ret = SendGpsCmd(cmdstr, 1000))		// error on first attempt
		{
			if(ret = SendGpsCmd(cmdstr, 1000))	// error on 2nd attempt
			{
				LogEntry("Error sending GPS command");
		...
		if(ret == BCSUCCESS)
		{
			...
			cmdstr = NULL;
			if(GetGpsReply(reply, 1000) == -1) fprintf(stderr, "%s: Timeout error waiting for GPS reply\r\n", MODULE_NAME);
			LogEntry(reply);
			i++;
		}
	}
...
int OpenLogFile(char *name)
{
	...
	{
		LogEntry("Error generating log file name");
	...
	{
		LogEntry("Error opening log file");
	...
	else
	{
		if(verbose) 
		{
			...
			fflush(stdout);
			LogEntry("Opened log file");
		}
		if(curfilename!=NULL) free(curfilename);
		curfilename = malloc(strlen(strName)+1);
...
int CloseLogFile(void)
{
	...
	if(fp)
	{
		if(err = fclose(fp))
		{
			LogEntry("Error closing log file");
...
*/	
	...
	if(n != strlen(cmdstr)) 
	{
		fprintf(stderr, "%s: Error writing to GPS command port.\r\n",MODULE_NAME);
		return(-2);					// Some sort of write error occurred
	}
...
int GetGpsReply(char *replybuf, int timeout)
{
	...
	if(nowtime >= endtime)	
	{
		if(verbose) printf("%s: GPS Reply Timeout\r\n", MODULE_NAME); // Timed out without receiving prompt
		return(-1);		
	}
...
int LogEntry(char *entry)
{
	...
	dir = opendir(DEBUG_OUTPUT);
	if(!dir)
		if(ENOENT == errno)		// Directory not found
		{
			ret = mkdir(DEBUG_OUTPUT, 0777);		// create directory
			if(ret)
				if(errno != EEXIST)		// Existing directories not an error, anything else is
				{
					closedir(dir);
	...
	epoch = time(NULL);
	now = gmtime(&epoch);
	sprintf(filename,"%4d-%02d-%02d_roc_log.txt", (now->tm_year)+1900, (now->tm_mon)+1, now->tm_mday);
	sprintf(fullpath,"%s%s", DEBUG_OUTPUT, filename);
	
	sprintf(tagged_entry, "%4d-%02d-%02d-%02d%02d%02d %s\r\n", (now->tm_year)+1900, (now->tm_mon)+1, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec, entry);
	filep=fopen(fullpath, "a");
	t=fwrite(tagged_entry, 1, strlen(tagged_entry), filep);
	fclose(filep);
	closedir(dir);
	
	if(t!=strlen(tagged_entry)) return(BCERROR);
	else return(BCSUCCESS);
}


/* $GPRMC,123519.00,A,4807.038,N,01131.000,E,022.4,084.4,210307,003.1,E,N*53
	0th Field = "$GPRMC"
	1st Field = UTC HHMMSS.ss
	2nd Field = A for valid fix, V for invalid fix
	3rd Field = DDMM.mmm latitude
	4th Field = N/S Latitude
	5th Field = DDDMM.mmm longitude
	6th Field = E/W longitude
	7th Field = Speed (kts)
	8th Field = CMG (degrees)
	9th Field = Date DDMMYY
	10th Field = Magnetic variation (deg)
	11th Field = Magnetic variation direction E/W
	12th Field = Positioning mode A/D/E/M/N
	13th Field = "*xx" Checksum
...
{
	int yr,mon,day,hr,min,sec,fracsec;
	char valid,latdir,londir,magdir,mode;
	float lat,lon,speed,cmg,magvar;
	int read;
	char cmdstr[64];
	
	sprintf(cmdstr, "");
	yr=mon=day=hr=min=sec=fracsec=read=0;
	valid=latdir=londir=magdir=mode=0;
	lat=lon=speed=cmg=magvar=0.0;
	
	if(rmc)
	{
		read = sscanf(rmc, "$GPRMC,%2d%2d%2d.%d,%c,%f,%c,%f,%c,%f,%f,%2d%2d%2d,%f,%c,%c",&hr,&min,&sec,&fracsec,&valid,&lat,&latdir,&lon,&londir,&speed,&cmg,&day,&mon,&yr,&magvar,&magdir,&mode);
		if((valid!='A') && (valid!='V'))	// It has to be one of those 2 characters.
		{
			fprintf(stderr, "%s: Error parsing RMC message, time not set\r\n", MODULE_NAME);
			LogEntry("Error parsing RMC message, time not set");
			return(-2);
		}
		else
		{
			...
			if(valid=='A')		// Valid GPS fix
			{
				...
				system(cmdstr);
				LogEntry("Set ROC Time:");
				LogEntry(cmdstr);
				return(BCSUCCESS);
			}
			else				// Invalid GPS Fix
			{
				LogEntry("RMC message not valid, time not set");
				return(-1);
			}
		}
	}
