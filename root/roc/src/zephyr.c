...
//			if (waitingIM) {		// Commented out 18-sep-18 by DMJ
//				waitingIM -= 1;		// I think we want to set waitingIM to 0 only when we
//			}						// switch to FL mode
			if(safeTimeout)	{ 
				safeTimeout -= 1;
...
//		if(time(NULL) > ackTimeout) waitingAck=0;
		...
		//	    if (verbose) printf("%s: FL Set Safe Output to Low\r\n", MODULE_NAME);
				...
				{
					sprintf(msg,"Switching to mode %d",InstMode);
					LogEntry(msg);
					modeSwitch=0;
// Sepentrio Wakeup
					if (wakeUpFlag) {
...
//								gps_power(OFF);
		...
		//	    if (verbose) printf("%s: SB Set Safe Output to Low\r\n", MODULE_NAME);
				...
				{
					if (strstr(nextFile,"list.txt")) {		// I think this whole block goes away now DMJ-18-sep-18
						TCflag = 1;
						if(TClastPart==TCnumParts)		// we finished the previous file, starting a new one
						{
							strcpy(TCfileOffload, nextFile);
						...
						TCflag = 0;
						if(lastPart==numParts)	{	// we finished the previous file
//							dataTm = 0;
//							SendMsg(TM, "", 0);
...
//      jhaase@ucsd.edu commented out 2018-09-13
//	if(memmem(payload, len, "ls", strlen("ls")))	// do "ls -1" on queue and put results in file for offload
//	{
//		sprintf(cmdstr,"ls -lt /data/roc/20180813 > /data/roc/queue/list.txt");
//		doit=1;
//	}
...
int LogEntry(const char *entry)
{
	...
	epoch = time(NULL);
	now = gmtime(&epoch);
	sprintf(filename,"%4d-%02d-%02d_zephyr_log.txt", (now->tm_year)+1900, (now->tm_mon)+1, now->tm_mday);
	...
	fp=fopen(fullpath, "a");
	t=fwrite(tagged_entry, 1, strlen(tagged_entry), fp);
	fclose(fp);
	closedir(dir);
